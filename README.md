...
2. В БД банка выполняется команда BEGIN TRANSACTION. Система отмечает начало атомарной операции списания средств со счета клиента. Создается контекст транзакции и журнал отката.
